export default (() => {
  ...
  class el extends HTMLElement {
    ...
    connectedCallback() {
      ...
      if (this.loadMoreEl) {
        this.loadMoreEl.onclick = () => this.getData().catch(console.error);
      }
    }
    setAttibutes() {
    ...
    async getData() {
        ...
        `https://firestore.googleapis.com/v1/projects/jaspero-jms/databases/(default)/documents:runQuery`,
        {
          ...
          body: JSON.stringify({
            structuredQuery: {
              ...
              limit: this.limit + 1,
              offset: this.offset,
              orderBy: [{ field: { fieldPath: 'publishedOn' }, direction: 'DESCENDING' }]
            }
          })
        }
      );
      ...
      res
        .slice(0, this.limit)
        .forEach((doc) => this.createCard(doc.document.fields, doc.document.name.split('/').pop()));
    }
    createCard(blog, id) {
      const item = {
        ...
        link: this.path + id,
        publishedOn: new Date(parseInt(blog.publishedOn.integerValue, 10)).toLocaleDateString(),
